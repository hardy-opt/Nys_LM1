function [mu, sg] = mean_runN(dataset, method, lambda, eta, rho, EPOCHS, run, COL, BS,delta)
    i=0;
    ratio=0.1;
    K = containers.Map();
    ...
    for j = 1:run
        if rho > -1
            if COL>0
                ...
                Name = strcat(dataset, sprintf('K%d_B%d_%s_%.1e_R_%.1e_rho_%.1e_delta_%.1e_run_%d.mat', COL, BS, method, eta, lambda, rho, delta,j));
